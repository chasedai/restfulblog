...
public class PostsResources {
	...
	@Path("{id}")
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public void remove(@PathParam("id") String id, @HeaderParam("authorization") String authString){
	...
	private boolean isUserAuthenticated(String authString){
        ...
        if(decodedAuth.substring(0, id).equals("Chase")&&decodedAuth.substring(id+1, decodedAuth.length()).equals("daiqi123")){
        	return true;
        }else{
        	return false;
        }
